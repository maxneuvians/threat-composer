...
function createLabelFunction(columnName: any) {
  return ({ sorted, descending }: {sorted: any; descending: any}) => {
...
export const columnDefinitions = [
  {
    id: 'id',
    header: 'id',
    cell: (item: any) => <Link href={`#${item.id}`}>{item.id}</Link>,
  ...
  },
  {
    id: 'priority',
    header: 'priority',
    cell: (item: any) => <Badge color={item.priority === 'High' ? 'red' : item.priority === 'Medium' ? 'blue' : item.priority ==='Low' ? 'green':'grey'}>{item.priority || 'Priority Not Set'}</Badge>,
