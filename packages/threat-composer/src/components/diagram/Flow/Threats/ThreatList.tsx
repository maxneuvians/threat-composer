...
export default memo(({ threats, component, changeHandler, onThreatListView }: {
  ...
  const updateData = (key: any, value: any, defaultValue: any) => {
    if (component) {
  ...
  const {
    ...
    threats.map((item: any) => {
      ...
      output.priority = item.metadata?.find((m: any) => m.key === 'Priority')?.value;
      output.stride = item.metadata?.find((m: any) => m.key === 'STRIDE')?.value.sort().join(',');
      return output;
    }),
    {
      propertyFiltering: {
